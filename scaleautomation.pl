...
if ( defined($session_file) && defined($outfile) && defined($gain) ) {
    if ( is_integer($gain) ) {
        ...
        if ( $gain > 60 || $gain < -60 ) {
            die "Gain must be within -60 and 60";
        }
        ...
        for ( $i = 3 ; $i < $numargs ; $i++ ) {
            ...
            @nodelist =
              $root->findnodes(
"/Session/Routes/Route[\@name='$track']/Processor[\@type='amp']/Automation/AutomationList[\@automation-id='gain']/events"
              );
            ...
            if (@nodelist) {
                ...
                $nodelist[0]->firstChild->replaceNode(
